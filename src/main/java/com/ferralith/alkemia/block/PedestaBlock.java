...
public class PedestaBlock extends BaseEntityBlock {
    private static final VoxelShape SHAPE = Stream.of(
            Block.box(0, 0, 0, 16, 2, 16),
            Block.box(1, 11, 1, 15, 15, 15),
            Block.box(3, 2, 3, 13, 11, 13)
    ).reduce((v1, v2) -> Shapes.join(v1, v2, BooleanOp.OR)).get();
    private static final MapCodec<PedestaBlock> CODEC = simpleCodec(PedestaBlock::new);

    public PedestaBlock(Properties properties) {
        super(properties);
    }
    ...
    protected ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
        if (level.getBlockEntity(pos) instanceof PedestalBlockEntity entity) {
